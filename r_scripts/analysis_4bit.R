...
for (i in input_files) {
  ...
  file_id <- gsub('./model_results/|_results.tsv|llama2_13b_4bit_', '', i) # adapt accordingly to folder/file name
...
write.table(results_wide, file = 'model_4bit_results_raw.tsv', row.names = FALSE,
...
# Fig. 5A: Global accuracy

  # Load model results
  results_wide <- read.delim('model_4bit_results_raw.tsv')
  ...
  for (i in accuracy_global_level$Model) {
    ...
    pdf(paste('fig5a_', i, '.pdf', sep = ''), width = (9 / 2.54) * 0.5, height = (6.75/ 2.54) * 0.5)
...
# Fig. 5B: Feature level accuracy
  ...
  pdf("fig5b.pdf", width = (12 / 2.54) * 0.95, height = (10 / 2.54))
...
# Fig. 5A + 5B (Supplementary Table): Precision, Recall, F1-Score 
  case_ids <- case_filter()
  ...
  write.table(stats_global_lvl, file = 'f1_llama2_global.tsv', row.names = FALSE,
              col.names = TRUE, sep = '\t', quote = FALSE)
  ...
  write.table(stats_feature_lvl, file = 'f1_llama2_parameter.tsv', row.names = FALSE,
              col.names = TRUE, sep = '\t', quote = FALSE)
  
  
# Fig. 5C: JSON generation (Valid/Total JSON)
  ...
  # Dumbbell Plot
  new_names <- c("llama2_13b_4bit_cot" = "CoT",
                 "llama2_13b_4bit_cov" = "CoV",
                 "llama2_13b_4bit_fewshot" = "Few-shot",
                 "llama2_13b_4bit_rephrase" = "Rephrase",
                 "llama2_13b_4bit_zeroshot" = "Zero-shot")
  ...
  fig5c <- ggplot(tmp, aes(x=reorder(Model, desc(Model)), y=eng)) +
...
# Fig. 5D: Average time per report
  ...
  time_distribution <- time_distribution %>%
    mutate(model_id = str_replace_all(model_id, c('llama2_13b_4bit_' = '', '_json' = ''))) %>%
  ...
  fig5d <- ggplot(time_distribution, aes(x = time, y = model_id, fill = model_id)) +
