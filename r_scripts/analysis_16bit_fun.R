...
case_filter <- function(data_base = 'Database_example.xlsx',
                        report_file = './report_data_example.xlsx') {
...
# Function to read Excel file and fill NA values in the Barcode column with the previous non-NA value
import_reports <- function(report_filename) {
  
  report_df <- read_excel(report_filename)
  colnames(report_df) <- c("ID", "Report")
  out <- fill(report_df, ID, .direction = "down")
